...
def _apa_table(df, title="Table"):
    ...
    st.markdown(f"**{title}**")
    styled = df.style.set_table_attributes('style="border-collapse: collapse; width: 100%;"')
    ...
    styled = styled.set_table_styles([{
        'selector': 'th',
        'props': [('background-color', '#1e1e1e'), ('color', 'white'), ('font-weight', 'bold'),
...
def _heatmap_fig(data, x_labels, y_labels, title="", colorscale="Viridis", text_template="%{text}"):
...
def descriptive_tabulation():
    ...
    tab = _tab_buttons([
        "Frequency Tables", "Relative Frequency Tables", "Cumulative Frequency Tables",
        "Descriptive Statistics Tables", "Grouped Summary Tables", "Pivot Tables",
...
def _generate_sample_data(n, dist="Normal", seed=42):
    ...
    if dist == "Normal":
        ...
        return np.concatenate([np.random.normal(30, 8, n // 2), np.random.normal(70, 8, n - n // 2)])
    return np.random.normal(50, 15, n)
...
def _freq_table_widget():
    ...
    with c1:
        ...
        dist = st.selectbox("Distribution", ["Normal", "Skewed", "Uniform", "Bimodal"], key="ft_dist")
    data = _generate_sample_data(n, dist)
    counts, edges = np.histogram(data, bins=bins)
    labels = [f"{edges[i]:.1f}–{edges[i+1]:.1f}" for i in range(len(edges)-1)]
    df = pd.DataFrame({"Class Interval": labels, "Frequency": counts, "Cumulative Freq": np.cumsum(counts)})
    with c2:
        _apa_table(df, "Frequency Distribution Table")
        fig = go.Figure(data=[go.Bar(x=labels, y=counts, marker_color="#4C78A8")])
        fig.update_layout(template="plotly_dark", height=300, margin=dict(l=10, r=10, t=10, b=80),
                          xaxis_title="Class", yaxis_title="Frequency")
...
def _rel_freq_table_widget():
    ...
    with c1:
        ...
        dist = st.selectbox("Distribution", ["Normal", "Skewed", "Uniform", "Bimodal"], key="rf_dist")
    data = _generate_sample_data(n, dist)
    counts, edges = np.histogram(data, bins=bins)
    labels = [f"{edges[i]:.1f}–{edges[i+1]:.1f}" for i in range(len(edges)-1)]
    rel_freq = counts / n
    df = pd.DataFrame({
        "Class Interval": labels, "Frequency": counts,
    ...
    with c2:
        ...
        fig = go.Figure(data=[go.Bar(x=labels, y=rel_freq * 100, marker_color="#E45756",
                                      hovertemplate="%{x}<br>Percentage: %{y:.1f}%<extra></extra>")])
        fig.update_layout(template="plotly_dark", height=300, margin=dict(l=10, r=10, t=10, b=80),
                          xaxis_title="Class", yaxis_title="Percentage (%)")
        st.plotly_chart(fig, use_container_width=True)
...
def _cum_freq_table_widget():
    ...
    with c1:
        ...
        dist = st.selectbox("Distribution", ["Normal", "Skewed", "Uniform", "Bimodal"], key="cf_dist")
    data = _generate_sample_data(n, dist)
    counts, edges = np.histogram(data, bins=bins)
    labels = [f"{edges[i]:.1f}–{edges[i+1]:.1f}" for i in range(len(edges)-1)]
    ...
    with c2:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=labels, y=cum_freq, mode="lines+markers",
                                  name="Cumulative Freq", marker_color="#4C78A8"))
        fig.add_trace(go.Scatter(x=labels, y=rel_cum * 100, mode="lines+markers",
                                  name="Cumulative %", marker_color="#E45756", yaxis="y2"))
        fig.update_layout(template="plotly_dark", height=300, margin=dict(l=10, r=10, t=10, b=80),
                          xaxis_title="Class", yaxis_title="Cumulative Frequency",
...
def _desc_stats_table_widget():
    ...
    with c1:
        ...
        dist = st.selectbox("Distribution", ["Normal", "Skewed", "Uniform", "Bimodal"], key="ds_dist")
    ...
    df = pd.DataFrame({
        "Statistic": ["n", "Mean", "Median", "Std Dev", "SE", "Skewness", "Kurtosis",
                       "Q1 (25th)", "Q3 (75th)", "IQR", "Min", "Max",
                       "95% CI Lower", "95% CI Upper"],
        "Value": [f"{len(data)}", f"{mean_v:.3f}", f"{median_v:.3f}", f"{std_v:.3f}",
                   f"{se_v:.3f}", f"{skew_v:.3f}", f"{kurt_v:.3f}",
                   f"{q1:.3f}", f"{q3:.3f}", f"{q3 - q1:.3f}",
                   f"{data.min():.3f}", f"{data.max():.3f}",
                   f"{mean_v - ci:.3f}", f"{mean_v + ci:.3f}"]
    })
    with c2:
...
def _grouped_summary_widget():
    ...
    for name, vals in group_data.items():
        stats_list.append({
            "Group": name, "n": len(vals), "Mean": f"{np.mean(vals):.2f}",
            "SD": f"{np.std(vals, ddof=1):.2f}", "SE": f"{np.std(vals, ddof=1)/np.sqrt(len(vals)):.2f}",
            "Median": f"{np.median(vals):.2f}", "IQR": f"{np.percentile(vals,75)-np.percentile(vals,25):.2f}"
        })
    ...
    with c2:
        ...
        for i, (name, vals) in enumerate(group_data.items()):
            fig.add_trace(go.Box(y=vals, name=name, marker_color=px.colors.qualitative.Plotly[i]))
        fig.update_layout(template="plotly_dark", height=300, margin=dict(l=10, r=10, t=10, b=10))
        st.plotly_chart(fig, use_container_width=True)
...
def _pivot_table_widget():
    ...
    with c1:
        ...
        agg_func = st.selectbox("Aggregation", ["Count", "Sum", "Mean", "Row %", "Col %"], key="pt_agg")
    np.random.seed(42)
...
def _dist_summary_widget():
    ...
    with c1:
        ...
        dist = st.selectbox("Distribution", ["Normal", "Skewed", "Uniform", "Bimodal"], key="dss_dist")
    data = _generate_sample_data(n, dist)
    ...
    with c2:
        ...
        fig.add_trace(go.Box(y=data, name="Data", boxmean="sd", marker_color="#4C78A8"))
        for p in [25, 50, 75]:
            fig.add_hline(y=np.percentile(data, p), line_dash="dash",
...
def cross_tabulation():
    ...
    tab = _tab_buttons([
        "2×2 Contingency Tables", "RxC Contingency Tables",
        "Proportion Tables", "Row Percentage Tables", "Column Percentage Tables",
        "Marginal Totals", "Expected Frequency Tables"
    ], "cross_tab")
    ...
    if tab == "2×2 Contingency Tables":
        ...
        _col_pct_widget()
    elif tab == "Marginal Totals":
        _marginal_totals_widget()
...
def _twobytwo_widget():
    ...
    df_display = pd.DataFrame(table, index=["Group 1", "Group 2"], columns=["Success", "Failure"])
    ...
    with c2:
        ...
        if show_expected:
            ...
            exp_df = pd.DataFrame(expected, index=["Group 1", "Group 2"], columns=["Success", "Failure"])
            st.caption("Expected Frequencies (under independence)")
            st.dataframe(exp_df.style.format("{:.1f}"), use_container_width=True)
        if show_heat:
            fig = _heatmap_fig(table, ["Success", "Failure"], ["Group 1", "Group 2"],
                                colorscale="Viridis")
            st.plotly_chart(fig, use_container_width=True)
    ...
    with st.expander(" Observed vs Expected Comparison", expanded=True):
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Bar(name="Observed", x=cells, y=obs_flat, marker_color="#4C78A8"))
        fig2.add_trace(go.Bar(name="Expected", x=cells, y=exp_flat, marker_color="#E45756"))
        fig2.update_layout(template="plotly_dark", height=300, barmode="group",
                           xaxis_title="Cell", yaxis_title="Count")
        st.plotly_chart(fig2, use_container_width=True)
        deviations = obs_flat - exp_flat
        contrib = deviations**2 / exp_flat
        comp_df = pd.DataFrame({
            "Cell": cells, "Observed": obs_flat, "Expected": np.round(exp_flat, 1),
...
def _rxc_widget():
    ...
    with c1:
        ...
        effect = st.slider("Association Strength", 0.0, 3.0, 1.0, 0.1, key="rxc_eff",
    ...
    with col2:
        st.metric("p-value", f"{p_val:.4f}")
    with col3:
        cramer = np.sqrt(chi2_val / (base.sum() * min(r-1, c-1))) if base.sum() > 0 else 0
        st.metric("Cramér's V", f"{cramer:.4f}")
...
def _proportion_table_widget():
    ...
    with c2:
        ...
        fig = _heatmap_fig(props, col_names, row_names, colorscale="Blues",
                            text_template="%{text:.3f}")
        st.plotly_chart(fig, use_container_width=True)
    st.info("Proportions represent each cell as a fraction of the grand total. They sum to 1.0.")
...
def _row_pct_widget():
    ...
    with c2:
        ...
        fig = _heatmap_fig(row_pct, col_names, row_names, colorscale="Reds",
                            text_template="%{text:.1f}%")
        st.plotly_chart(fig, use_container_width=True)
    st.info("**Row Percentages**: Each row sums to 100%. Compares distribution across columns WITHIN each row group.")
...
def _col_pct_widget():
    ...
    with c2:
        ...
        fig = _heatmap_fig(col_pct, col_names, row_names, colorscale="Greens",
                            text_template="%{text:.1f}%")
        st.plotly_chart(fig, use_container_width=True)
    st.info("**Column Percentages**: Each column sums to 100%. Compares distribution across rows WITHIN each column level.")


def _marginal_totals_widget():
    c1, c2 = st.columns([1, 2])
    with c1:
        rows = st.selectbox("Number of Rows", [2, 3, 4], index=1, key="marg_r")
        cols = st.selectbox("Number of Columns", [2, 3, 4], index=1, key="marg_c")
    np.random.seed(42)
    r, c = int(rows), int(cols)
    table = np.random.randint(5, 50, (r, c))
    ...
    df["Row Total"] = row_tot
    df["Row %"] = [f"{x:.1f}%" for x in row_tot / n_tot * 100]
    marg_row = pd.DataFrame([list(col_tot) + [n_tot, "100%"]],
                             index=["Column Total"],
                             columns=list(col_names) + ["Row Total", "Row %"])
    df = pd.concat([df, marg_row])
    with c2:
        _apa_table(df, "Contingency Table with Marginal Totals")
        col_tot_pct = col_tot / n_tot * 100
        row_tot_pct = row_tot / n_tot * 100
        fig = go.Figure()
        fig.add_trace(go.Bar(name="Row Totals", x=row_names, y=row_tot, marker_color="#4C78A8"))
        fig.add_trace(go.Bar(name="Column Totals", x=col_names, y=col_tot, marker_color="#E45756"))
        fig.update_layout(template="plotly_dark", height=250, barmode="group",
                          xaxis_title="", yaxis_title="Frequency")
        st.plotly_chart(fig, use_container_width=True)
    with st.expander(" Educational Notes - Marginal Totals", expanded=True):
        st.info("""
        **Marginal Totals** are the sums of rows and columns.
        - **Row margins**: distribution of the row variable (e.g., group sizes)
        - **Column margins**: distribution of the column variable (e.g., outcome prevalence)
        - Under independence: expected cell = (row total × col total) / grand total
        - Marginal totals are essential for calculating expected frequencies and Chi-square
        """)
...
def _expected_freq_widget():
    ...
    row_names = [f"Row {chr(65+i)}" for i in range(r)]
    col_names = [f"Col {j+1}" for j in range(c)]
    tab_view = st.radio("View", ["Observed", "Expected", "Side-by-Side"],
    ...
    with c2:
        if tab_view == "Observed":
            ...
            fig.add_trace(go.Bar(name="Observed", x=cells, y=obs_flat, marker_color="#4C78A8"))
            fig.add_trace(go.Bar(name="Expected", x=cells, y=exp_flat, marker_color="#E45756"))
...
def diagnostic_accuracy():
    ...
    with c2:
        df = pd.DataFrame(matrix, index=["Test +", "Test -"],
                           columns=["Condition +", "Condition -"])
        _apa_table(df, "Confusion Matrix")
        if show_heat:
            fig = _heatmap_fig(matrix, ["Condition +", "Condition -"], ["Test +", "Test -"],
                                colorscale="RdBu_r")
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col3:
        st.metric("LR+", f"{lr_plus:.3f}" if lr_plus != float("inf") else "∞",
                   help="Sensitivity / (1 - Specificity)")
    with col4:
        st.metric("LR-", f"{lr_minus:.3f}" if lr_minus != float("inf") else "∞",
                   help="(1 - Sensitivity) / Specificity")

    diag_df = pd.DataFrame({
        "Metric": ["Sensitivity", "Specificity", "PPV", "NPV", "Prevalence",
                    "Accuracy", "LR+", "LR-"],
        "Formula": ["TP/(TP+FN)", "TN/(TN+FP)", "TP/(TP+FP)", "TN/(TN+FN)",
                     "(TP+FN)/N", "(TP+TN)/N", "Se/(1-Sp)", "(1-Se)/Sp"],
        "Value": [f"{sensitivity:.4f}", f"{specificity:.4f}", f"{ppv:.4f}",
                  f"{npv:.4f}", f"{prevalence:.4f}", f"{accuracy:.4f}",
    ...
    with st.expander(" Bayesian Updating Table", expanded=True):
        ...
        pre_test_odds = prevalence / (1 - prevalence) if prevalence < 1 else float("inf")
        post_test_odds_pos = pre_test_odds * lr_plus if lr_plus != float("inf") else float("inf")
        post_test_prob_pos = post_test_odds_pos / (1 + post_test_odds_pos) if post_test_odds_pos != float("inf") else 1.0
        ...
        bayes_df = pd.DataFrame({
            ...
            "Probability": [f"{prevalence:.4f}", f"{post_test_prob_pos:.4f}", f"{post_test_prob_neg:.4f}"],
            "Odds": [f"{pre_test_odds:.4f}" if pre_test_odds != float("inf") else "∞",
                     f"{post_test_odds_pos:.4f}" if post_test_odds_pos != float("inf") else "∞",
...
def agreement_tables():
    ...
    tab = _tab_buttons(["Observer Agreement Matrix", "Cohen's Kappa Tables",
                         "Reliability Summary Tables"], "agree_tab")
...
def _observer_agreement_widget():
    ...
    with c1:
        ...
        strength = st.slider("Agreement Strength", 0.0, 1.0, 0.7, 0.05, key="oa_str",
...
def _kappa_widget():
    ...
    se_kappa = math.sqrt(p_o * (1 - p_o) / (n_tot * (1 - p_e)**2)) if n_tot > 0 and p_e < 1 else 0
    z_kappa = kappa / se_kappa if se_kappa > 0 else 0
    p_kappa = 2 * (1 - stats.norm.cdf(abs(z_kappa)))
    df = pd.DataFrame(table, index=["Rater1: Yes", "Rater1: No"],
                       columns=["Rater2: Yes", "Rater2: No"])
    with c2:
        _apa_table(df, "Agreement Table (2×2)")
        fig = _heatmap_fig(table, ["Rater2: Yes", "Rater2: No"], ["Rater1: Yes", "Rater1: No"],
                            colorscale="Blues")
        st.plotly_chart(fig, use_container_width=True)
    col1, col2, col3, col4 = st.columns(4)
    with col1:
    ...
    with col3:
        st.metric("Cohen's Kappa", f"{kappa:.4f}")
    with col4:
        kappa_label = "Almost Perfect" if kappa > 0.8 else "Substantial" if kappa > 0.6 else "Moderate" if kappa > 0.4 else "Fair" if kappa > 0.2 else "Slight"
...
def _reliability_widget():
    c1, c2 = st.columns([1, 2])
    with c1:
        icc_type = st.selectbox("ICC Type", ["ICC(1,1) - Single rater, absolute",
        ...
        reliability = st.slider("True Reliability", 0.0, 1.0, 0.6, 0.05, key="icc_rel",
    ...
    ratings = np.array([true_scores + np.random.normal(0, noise, ns) for _ in range(nr)]).T
    ...
    with c2:
        _apa_table(rating_df.head(10), f"Reliability Data (showing first 10 of {ns} subjects)")
    ms_between = np.var(ratings.mean(axis=1), ddof=1) * ns
    ms_within = np.mean(np.var(ratings, axis=1, ddof=1))
    icc_est = ms_between / (ms_between + ms_within) if (ms_between + ms_within) > 0 else 0
    col1, col2, col3 = st.columns(3)
    with col1:
...
def regression_tables():
    ...
    tab = _tab_buttons(["Coefficient Tables", "Odds Ratio Tables",
                         "Model Fit Tables", "ANOVA Tables",
                         "Residual Summary Tables"], "reg_tab")
...
def _coeff_table_widget():
    c1, c2 = st.columns([1, 2])
    with c1:
        n_predictors = st.selectbox("Number of Predictors", [1, 2, 3, 4], index=1, key="coef_np")
    ...
    stars = ["***" if p < 0.001 else "**" if p < 0.01 else "*" if p < 0.05 else "" for p in p_vals]
    ci_low = beta - 1.96 * se
    ci_high = beta + 1.96 * se
    df = pd.DataFrame({
        "Predictor": names, "β": np.round(beta, 4),
        "SE": np.round(se, 4), "t": np.round(t_vals, 3),
    ...
    with c2:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=beta, y=names, mode="markers",
        ...
        fig.update_layout(template="plotly_dark", height=300, margin=dict(l=10, r=10, t=10, b=10),
                          xaxis_title="Coefficient (95% CI)", yaxis_title="")
        st.plotly_chart(fig, use_container_width=True)
...
def _odds_ratio_table_widget():
    c1, c2 = st.columns([1, 2])
    with c1:
        n_pred = st.selectbox("Number of Predictors", [1, 2, 3, 4], index=1, key="or_np")
    ...
    df = pd.DataFrame({
        "Predictor": names, "OR": np.round(or_vals, 3),
        "95% CI": [f"[{l:.3f}, {h:.3f}]" for l, h in zip(ci_low, ci_high)],
        "z": np.round(z_vals, 3), "p-value": np.round(p_vals, 4),
        "": ["***" if p < 0.001 else "**" if p < 0.01 else "*" if p < 0.05 else "" for p in p_vals]
    })
    with c2:
        _apa_table(df, "Odds Ratio Table (Logistic Regression)")
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=or_vals, y=names, mode="markers",
                                  marker=dict(size=10, color="#E45756"),
                                  error_x=dict(type="data", array=[or_vals[i] - ci_low[i] for i in range(npred)],
                                               symmetric=False),
                                  hovertemplate="%{y}: OR=%{x:.2f}<extra></extra>"))
        fig.add_vline(x=1, line_dash="dash", line_color="gray", opacity=0.5)
        fig.update_layout(template="plotly_dark", height=300, margin=dict(l=10, r=10, t=10, b=10),
                          xaxis_title="Odds Ratio (95% CI)", yaxis_title="")
        st.plotly_chart(fig, use_container_width=True)
...
def _model_fit_widget():
    ...
    df = pd.DataFrame({
        "Metric": ["R²", "Adjusted R²", "F-statistic", "df (model)", "df (residual)",
                    "p-value (F)", "AIC", "BIC", "RMSE"],
        "Value": [f"{r2:.4f}", f"{r2_adj:.4f}", f"{f_val:.3f}",
                   f"{n_predictors}", f"{n - n_predictors - 1}",
                   f"{p_f:.4e}", f"{aic:.2f}", f"{bic:.2f}", f"{rmse:.4f}"]
    })
    with c2:
        _apa_table(df, "Model Fit Summary")
        fig = go.Figure(data=go.Indicator(
            mode="gauge+number+delta", value=r2 * 100,
            title={"text": "R² (%)"}, delta={"reference": 50},
            gauge={"axis": {"range": [0, 100]},
                   "bar": {"color": "#4C78A8"},
                   "steps": [{"range": [0, 25], "color": "rgba(255,0,0,0.2)"},
...
def _anova_table_widget():
    c1, c2 = st.columns([1, 2])
    with c1:
        n_groups = st.selectbox("Number of Groups", [2, 3, 4, 5], index=2, key="anova_ng")
        effect = st.slider("Effect Size (Cohen's f)", 0.0, 1.0, 0.4, 0.05, key="anova_f")
...
def _residual_table_widget():
    ...
    with c1:
        ...
        dist = st.selectbox("Error Distribution", ["Normal", "Heavy-tailed (t)", "Skewed"],
                            key="res_dist")
    np.random.seed(42)
    ...
    res_df = pd.DataFrame({
        "Observation": range(1, n + 1), "Observed": np.round(y, 3),
        "Predicted": np.round(y_pred, 3), "Residual": np.round(residuals, 3),
    ...
    with colr1:
        ...
        fig1.update_layout(template="plotly_dark", height=250, margin=dict(l=10, r=10, t=10, b=10),
    ...
    st.info(f"Shapiro-Wilk normality test: W = {shapiro_stat:.4f}, p = {shapiro_p:.4f}" +
             (" → Residuals appear normal" if shapiro_p > 0.05 else " → Residuals deviate from normality"))
...
def effect_size_tables():
    ...
    tab = _tab_buttons(["Cohen's d", "η² (Eta-squared)", "Cramér's V",
                         "Odds Ratios", "Relative Risk"], "es_tab")
...
def _cohens_d_widget():
    ...
    df = pd.DataFrame({
        "Metric": ["Cohen's d", "Pooled SD", "SE(d)", "95% CI Lower", "95% CI Upper", "Interpretation"],
        "Value": [f"{d:.4f}", f"{pooled_sd:.4f}", f"{se_d:.4f}",
                   f"{ci_low:.4f}", f"{ci_high:.4f}", label]
    })
    with c2:
        ...
        fig.add_trace(go.Scatter(x=x_range, y=y1, mode="lines", name="Group 1", fill="tozeroy",
                                  line=dict(color="#4C78A8")))
        fig.add_trace(go.Scatter(x=x_range, y=y2, mode="lines", name="Group 2", fill="tozeroy",
                                  line=dict(color="#E45756")))
        fig.update_layout(template="plotly_dark", height=300, margin=dict(l=10, r=10, t=10, b=10),
                          xaxis_title="Value", yaxis_title="Density")
        st.plotly_chart(fig, use_container_width=True)
    st.caption(f"d = {d:.3f}: {label} effect. Benchmarks: 0.2=Small, 0.5=Medium, 0.8=Large")
...
def _eta_sq_widget():
    ...
    omega_sq = (f_val**2 - (1 / (int(n_groups) * n))) / (1 + f_val**2) if int(n_groups) * n > 0 else 0
    label = "Large" if eta_sq >= 0.14 else "Medium" if eta_sq >= 0.06 else "Small" if eta_sq >= 0.01 else "Negligible"
    df = pd.DataFrame({
        "Metric": ["η²", "Partial η²", "ω²", "Cohen's f", "Interpretation"],
        "Value": [f"{eta_sq:.4f}", f"{eta_sq_partial:.4f}",
                   f"{omega_sq:.4f}", f"{f_val:.4f}", label]
    })
    with c2:
...
def _cramer_v_widget():
    ...
    n_total = table.sum()
    v = math.sqrt(chi2_val / (n_total * min(r - 1, c - 1))) if n_total > 0 and min(r - 1, c - 1) > 0 else 0
    df_rows = (r - 1) * (c - 1)
    p_val = 1 - stats.chi2.cdf(chi2_val, df_rows)
    label = "Large" if v >= 0.5 else "Medium" if v >= 0.3 else "Small" if v >= 0.1 else "Negligible"
    df = pd.DataFrame({
        "Metric": ["Cramér's V", "χ²", "df", "p-value", "Interpretation"],
        "Value": [f"{v:.4f}", f"{chi2_val:.3f}", f"{df_rows}", f"{p_val:.4f}", label]
    })
    with c2:
        _apa_table(df, "Cramér's V Effect Size")
    st.caption("Cramér's V benchmarks: 0.1=Small, 0.3=Medium, 0.5=Large. V = √(χ²/(N × min(r-1,c-1)))")
...
def _odds_ratio_es_widget():
    ...
    se_log = math.sqrt(1/a + 1/b + 1/c + 1/d) if all(x > 0 for x in [a, b, c, d]) else 0
    ...
    df = pd.DataFrame({
        "Metric": ["Odds Ratio", "95% CI (OR)", "Relative Risk", "ARR",
                    "RRR", "NNT"],
        "Value": [f"{or_val:.4f}" if or_val != float("inf") else "∞",
    ...
    with st.expander(" Educational Notes - Odds Ratio / Risk Ratio Explorer", expanded=True):
        st.info(f"""
...
def _relative_risk_widget():
    ...
    se_log_rr = math.sqrt(1/a_rr - 1/(a_rr + b_rr) + 1/c_rr - 1/(c_rr + d_rr)) if all(x > 0 for x in [a_rr, b_rr, c_rr, d_rr]) else 0
    ...
    df = pd.DataFrame({
        "Metric": ["Risk Exposed", "Risk Unexposed", "Relative Risk", "95% CI (RR)",
                    "Risk Difference", "NNT"],
        "Value": [f"{risk_exp:.4f}", f"{risk_unexp:.4f}", f"{rr_val:.4f}" if rr_val != float("inf") else "∞",
                  f"[{ci_low_rr:.4f}, {ci_high_rr:.4f}]",
                  f"{rd_val:.4f}", f"{nnt_val:.1f}" if nnt_val != float("inf") else "∞"]
    })
    with c2:
        _apa_table(df, "Relative Risk & Risk Difference")
        fig = go.Figure()
        fig.add_trace(go.Bar(name="Exposed", x=["Risk"], y=[risk_exp], marker_color="#E45756"))
        fig.add_trace(go.Bar(name="Unexposed", x=["Risk"], y=[risk_unexp], marker_color="#4C78A8"))
        fig.update_layout(template="plotly_dark", height=300, barmode="group",
                          yaxis=dict(range=[0, 1]), yaxis_title="Risk")
        st.plotly_chart(fig, use_container_width=True)
    st.caption(f"RR = {rr_val:.3f}: " +
               ("Increased risk" if rr_val > 1 else "Decreased risk" if rr_val < 1 else "No effect") +
               f" | NNT = {nnt_val:.1f}")
...
def educational_modules():
    ...
    module = _tab_buttons([
        "Frequency Table Explorer", "Cross-Tabulation Explorer",
        "Expected Frequency Explorer", "Odds Ratio / Risk Ratio Explorer",
        "Conditional Probability Explorer", "Bayesian Updating Table"
    ], "edu_mod")
...
def _freq_explorer():
    ...
    with c1:
        ...
        dist = st.selectbox("Distribution", ["Normal", "Skewed", "Uniform", "Bimodal"], key="fe_dist")
    ...
    fig = go.Figure()
    if show_freq:
        fig.add_trace(go.Bar(x=labels, y=counts, name="Absolute", marker_color="#4C78A8",
                              hovertemplate="%{x}<br>Freq: %{y}<extra></extra>"))
    if show_rel:
        fig.add_trace(go.Scatter(x=labels, y=rel * 100, mode="lines+markers",
                                  name="Relative %", marker_color="#E45756", yaxis="y2"))
    if show_cum:
        fig.add_trace(go.Scatter(x=labels, y=cum, mode="lines+markers",
                                  name="Cumulative", marker_color="#00CC96", yaxis="y3"))
    fig.update_layout(template="plotly_dark", height=350, margin=dict(l=10, r=10, t=10, b=80),
                      xaxis_title="Class", yaxis_title="Frequency",
    ...
    with c2:
        st.plotly_chart(fig, use_container_width=True)
    multi = pd.DataFrame({
        "Interval": labels, "Frequency": counts,
        "Relative": [f"{v*100:.1f}%" for v in rel],
        "Cumulative": cum, "Cum %": [f"{v*100:.1f}%" for v in cum / n]
    })
...
def _cross_tab_explorer():
    ...
    with c1:
        ...
        show_conditional = st.toggle("Show Conditional Probabilities", True, key="cte_cond")
    np.random.seed(42)
    ...
    with c2:
        ...
        if show_expected:
            ...
            df_exp = pd.DataFrame(np.round(expected, 1), index=row_names, columns=col_names)
    ...
    if show_conditional:
        ...
        with colc1:
            ...
            st.dataframe(pd.DataFrame(np.round(p_row_given_col, 3), index=row_names, columns=col_names),
                         use_container_width=True)
        with colc2:
            st.caption("P(Column | Row) — sums to 1 across each row")
            st.dataframe(pd.DataFrame(np.round(p_col_given_row, 3), index=row_names, columns=col_names),
                         use_container_width=True)
...
def _expected_freq_explorer():
    ...
    view = st.radio("View", ["Observed vs Expected", "Chi-square Contributions", "Heatmap"],
                     horizontal=True, key="efe_view")
    with c2:
        if view == "Observed vs Expected":
            ...
            fig = _heatmap_fig(chi2_contrib, col_names, row_names, colorscale="Reds",
                                text_template="%{text:.3f}")
            fig.update_layout(title="Cell-wise χ² Contribution (O-E)²/E", height=300)
            st.plotly_chart(fig, use_container_width=True)
            st.caption("Cells with contributions > 4 contribute substantially to the total χ²")
        else:
            fig = _heatmap_fig(observed, col_names, row_names, colorscale="Viridis")
            st.plotly_chart(fig, use_container_width=True)
    comp = pd.DataFrame({
    ...
    with col2:
        st.metric("p-value", f"{p_val:.4f}")
    with col3:
        cramer_v = math.sqrt(chi2_val / (observed.sum() * min(observed.shape[0]-1, observed.shape[1]-1))) if observed.sum() > 0 and min(observed.shape[0]-1, observed.shape[1]-1) > 0 else 0
...
def _odds_risk_explorer():
    ...
    with c2:
        df = pd.DataFrame(table, index=["Exposed", "Unexposed"],
    ...
    with col2:
        st.metric("Relative Risk (RR)", f"{rr_val:.3f}" if rr_val != float("inf") else "∞")
    with col3:
        st.metric("ARR", f"{arr_val:.4f}",
                   help="Absolute Risk Reduction = Risk(unexp) - Risk(exp)")
    with col4:
        st.metric("RRR", f"{rrr_val:.4f}" if rrr_val != float("inf") else "∞",
                   help="Relative Risk Reduction = ARR / Risk(unexp)")
    col1, col2 = st.columns(2)
    with col1:
        st.metric("NNT", f"{nnt_val:.1f}" if nnt_val != float("inf") else "∞",
                   help="Number Needed to Treat = 1/|ARR|")
    with col2:
        st.metric("Fisher's p", f"{p_fisher:.4f}")
    fig = go.Figure()
    fig.add_trace(go.Bar(name="Exposed (Risk)", x=["Risk"], y=[risk_exp * 100],
                          marker_color="#E45756"))
    fig.add_trace(go.Bar(name="Unexposed (Risk)", x=["Risk"], y=[risk_unexp * 100],
                          marker_color="#4C78A8"))
    fig.update_layout(template="plotly_dark", height=250, barmode="group",
                      yaxis=dict(range=[0, 100]), yaxis_title="Risk (%)",
...
def _cond_prob_explorer():
    ...
    with c1:
        ...
        direction = st.radio("Conditional Probability Direction",
                              ["P(A | B)", "P(B | A)", "P(A | ¬B)", "P(¬A | B)"],
                              horizontal=True, key="cp_dir")
    total = a + b + c_sl + d_sl
    ...
    with c2:
        df = pd.DataFrame(table, index=["A present", "A absent"],
...
def _bayesian_updater():
    ...
    c1, c2 = st.columns([1, 2])
    with c1:
        prevalence = st.slider("Prevalence (Pre-test P(Disease))", 0.001, 0.5, 0.1, 0.001,
                                format="%.3f", key="bayes_prev",
                                help="Probability of disease before testing")
        sensitivity = st.slider("Sensitivity (P(+|Disease))", 0.5, 0.999, 0.85, 0.001,
                                format="%.3f", key="bayes_sens",
                                help="True positive rate")
        specificity = st.slider("Specificity (P(-|No Disease))", 0.5, 0.999, 0.90, 0.001,
                                format="%.3f", key="bayes_spec",
                                help="True negative rate")
        n_pop = st.slider("Population Size (for visualization)", 100, 10000, 1000, 100,
                          key="bayes_n")
    p_d = prevalence
    p_nd = 1 - prevalence
    ...
    with c2:
        bayes_df = pd.DataFrame({
            "Step": ["Pre-test probability", "LR+", "LR-",
                      "Post-test prob (if +)", "Post-test prob (if -)"],
            "Value": [f"{prevalence:.4f}", f"{lr_plus:.3f}" if lr_plus != float("inf") else "∞",
                      f"{lr_minus:.3f}" if lr_minus != float("inf") else "∞",
                      f"{post_prob_pos:.4f}", f"{post_prob_neg:.4f}"]
        })
    ...
    st.caption(f"Population simulation (n ≈ {n_pop}): TP={tp}, FN={fn}, FP={fp}, TN={tn}")
    fig = go.Figure()
...
def apa_export():
    ...
    table_type = st.selectbox("Table Type", [
        "Descriptive Statistics", "Correlation Matrix", "Regression Results",
        "ANOVA Summary", "Contingency Table", "Effect Size Summary"
    ], key="apa_type")

    st.markdown("##### Formatting Options")
    col1, col2, col3 = st.columns(3)
    with col1:
        decimal_places = st.selectbox("Decimal Places", [2, 3, 4], index=1, key="apa_dec")
    with col2:
    ...
    if st.button("Generate APA Table", type="primary", use_container_width=True):
        ...
        if table_type == "Descriptive Statistics":
            ...
            df = pd.DataFrame({
                ...
                "95% CI": [f"[{np.mean(data)-1.96*np.std(data, ddof=1)/np.sqrt(100):.{decimal_places}f}, "
                           f"{np.mean(data)+1.96*np.std(data, ddof=1)/np.sqrt(100):.{decimal_places}f}]"],
                "Skew": [f"{stats.skew(data):.{decimal_places}f}"],
                "Kurtosis": [f"{stats.kurtosis(data, fisher=True):.{decimal_places}f}"]
            })
        elif table_type == "Correlation Matrix":
            n_vars = 4
            data = np.random.multivariate_normal([0]*n_vars, np.eye(n_vars)*0.5 + 0.5, 100)
            corr = np.corrcoef(data.T)
            df = pd.DataFrame(corr, columns=[f"V{i+1}" for i in range(n_vars)],
            ...
            stars = ["***" if p < 0.001 else "**" if p < 0.01 else "*" if p < 0.05 else "" for p in p_vals]
            data_dict = {"Predictor": ["Intercept"] + [f"X{i+1}" for i in range(n_pred)],
            ...
            df = pd.DataFrame({
                ...
                "Value": [f"{np.random.uniform(0.2, 0.8):.{decimal_places}f}",
                          f"{np.random.uniform(0.1, 0.5):.{decimal_places}f}",
                          f"{np.random.uniform(0.06, 0.14):.{decimal_places}f}"],
                "95% CI": [f"[{np.random.uniform(0.1, 0.3):.{decimal_places}f}, "
