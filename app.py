...
def _render_power_analysis():
    ...
    with col_left:
        ...
        analysis_type = st.selectbox(
            "Type of Analysis", ss_at_opts, index=0
        )
        ...
        with col_a:
            alpha_ss = st.slider(
                ...
                0.001, 0.10, 0.05, 0.001, format="%.3f",
            )
        with col_b:
            power_ss = st.slider(
                "Power (1 − β)",
                0.50, 0.99, 0.80, 0.01, format="%.2f",
            )
        tails_ss = st.radio(
            "Test Direction",
        ...
        if analysis_type == "One-sample Mean (t/z-test)":
            ...
            ss_params = {
                "type": "regression", "k": int(k_reg), "effect_size": f2_reg,
            }
            ...
            ss_params = {
                "type": "logistic", "k": int(k_log), "event_rate": ev_rate, "or": or_val,
            }
            ...
            ss_params = {
                "type": "mannwhitney", "effect_size": P_val,
                "ratio": ratio_mw, "are": are_val,
            }
            ...
            ss_params = {
                "type": "logrank", "hr": hr_val, "ratio": ratio_lr,
                "median_survival": med_val, "study_duration": dur_val,
            }
            ...
            ss_params = {
                "type": "cox", "hr": hr_val, "k": int(k_val),
                "sd_x": sd_val, "r2_x": r2_val, "event_rate": ev_val,
            }
            ...
            ss_params = {
                "type": "equiv", "margin": margin, "expected_diff": d_exp,
                "sd": sd_val, "ratio": ratio_eq,
                "equiv_param_type": equiv_param_type,
                "p1_eq": p1_eq, "p2_eq": p2_eq,
            }
            ...
            ss_params = {
                "type": "rm_anova", "effect_size": f_val, "k": int(k_val),
                "m": int(m_val), "rho": rho_val, "epsilon": eps_val,
            }
            ...
            ss_params = {
                "type": "twoway_anova", "f_a": f_a, "f_b": f_b, "f_ab": f_ab,
                "rows": int(r_val), "cols": int(c_val), "focus": focus,
            }
            ...
            ss_params = {
                "type": "roc_auc", "auc": auc_val, "null_auc": 0.5, "ratio": ratio_roc,
            }
            ...
            with c1:
                kappa_val = st.number_input(
                    "Expected Kappa", 0.01, 0.99, 0.6, 0.01,
                )
            with c2:
            ...
            ss_params = {
                "type": "kappa", "kappa": kappa_val, "null_kappa": null_kap,
                "raters": int(raters), "categories": int(cats),
            ...
            ss_params = {
                "type": "cluster_rct", "effect_size": d_val, "icc": icc_val,
                "cluster_size": int(m_val), "ratio": ratio_cl,
            }
            ...
            ss_params = {
                "type": "precision", "half_width": hw_val, "conf_level": cl_val,
                "param_type": ptype, "sd": sd_val, "prop": prop_val,
            }
            ...
            if method == "Rule of thumb":
                ...
                ss_params = {
                    "type": "pilot", "method": method, "n_per_group": int(npg_val),
                }
                ...
                sd_val = st.number_input("SD", 0.1, 100.0, 10.0, 0.1)
                ss_params = {
                    "type": "pilot", "method": method, "half_width": hw_val,
                    "conf_level": cl_val, "param_type": "Mean",
                    "sd": sd_val, "prop": 0.5,
                }
            else:
                main_n = st.number_input(
                    "Expected main study N", 10, 10000, 100, 1,
                )
                fraction = st.number_input("Fraction", 0.05, 0.5, 0.1, 0.01)
                ss_params = {
                    "type": "pilot", "method": method,
                    "fraction": fraction, "main_n": int(main_n),
                }
            ...
            ss_params = {
                "type": "wilcoxon_sr", "effect_size": pr_pos, "are": are_wsr,
            }
            ...
            ss_params = {
                "type": "kruskal", "k": int(k_kw), "effect_size": f_kw, "are": are_kw,
            }
            ...
            ss_params = {
                "type": "friedman", "k": int(k_fr), "m": int(m_fr),
                "w": w_fr, "are": are_fr,
            }
            ...
            ss_params = {
                "type": "fisher", "p1": p1_fish, "p2": p2_fish,
                "ratio": ratio_fish, "are": are_fish,
            }
            ...
            ss_params = {
                "type": "manova", "k": int(k_man), "dv": int(dv_man),
                "f2": f2_man, "rho": corr_man, "manova_test": manova_test,
            }
            ...
            sim_test = st.selectbox(
                ...
                ["Independent t-test (pooled)", "Welch's t-test",
            ...
            if sim_test in (
                ...
                ss_params = {
                    "type": "simulation", "sim_test": sim_test, "n_sim": int(n_sim),
                    "mu1": mu1_s, "mu2": mu2_s, "sd": sd_s,
                    "n_per": int(n_per_s), "dist": dist_type,
                }
            else:
                ...
                ss_params = {
                    "type": "simulation", "sim_test": sim_test, "n_sim": int(n_sim),
                    "p1_s": p1_s, "p2_s": p2_s, "n_per": int(n_per_s),
                }
        ...
        with st.expander("📐 Effect Size Converter"):
            ...
            conv_tab = st.radio(
                "Conversion",
                ["Means → d", "d ↔ r", "d ↔ OR", "η² ↔ f", "R² ↔ f²",
                 "2×2 Table → w/OR", "P(X>Y) ↔ d / Cliff's δ"],
                horizontal=True,
            ...
            if conv_tab == "Means → d":
                ...
                if n_t > 0:
                    ...
                    chi2_t = (
                        n_t * (abs(a_t * d_t - b_t * c_t) - n_t / 2) ** 2
                        / ((a_t + b_t) * (c_t + d_t) * (a_t + c_t) * (b_t + d_t))
                        if all(x > 0 for x in [a_t + b_t, c_t + d_t, a_t + c_t, b_t + d_t])
                        else 0
                    )
    ...
    with col_right:
        if st.button("Calculate Sample Size", use_container_width=True, type="primary"):
