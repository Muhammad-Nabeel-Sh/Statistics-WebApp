...
from core.data import FIELDS, rules
import pandas as pd
...
def _prepare_sunburst_data():
    rows = []
    for rule in rules:
        base = {k: rule.get(k, "any") for k in FIELDS}
        base["name"] = rule["name"]
        base["Objective"] = rule.get("Objective", "Unknown")
        rows.append(base)
...
@st.cache_resource(show_spinner="Building Sunburst Chart...")
def _get_sunburst_figure():
    df = _prepare_sunburst_data()
...
def build_sunburst_chart(rules_data=None, fields=None):
    fig = _get_sunburst_figure()
