...
""",
    ...
    {
        "title": "Sampling & Research Design",
        "content": """
...
""",
    ...
    {
        "title": "Distribution & Assumptions",
        "content": """
...
def render_glossary():
    ...
    if query:
        ...
        for section in SECTIONS:
            lines = [
                l.strip() for l in section["content"].strip().split("\n") if l.strip()
            ]
            for line in lines:
                if query in line.lower():
                    matched += 1
                    term_match = re.match(r":orange\[\*\*(.+?)\*\*\]:\s*(.*)", line)
                    if term_match:
                        term = term_match.group(1)
                        defn = term_match.group(2)
                        highlighted_defn = re.sub(
                            re.escape(query),
                            lambda m: f"**:orange-background[{m.group(0)}]**",
                            defn,
                            flags=re.IGNORECASE,
                        )
                        highlighted_term = re.sub(
                            re.escape(query),
                            lambda m: f"**:orange-background[{m.group(0)}]**",
                            term,
                            flags=re.IGNORECASE,
                        ...
                        st.markdown(
                            f":orange[**{highlighted_term}**]: {highlighted_defn}"
                        )
        if matched == 0:
            st.info(f'No glossary entries found matching "{query}".')
        else:
            st.caption(f"Found {matched} entr{'ies' if matched != 1 else 'y'}.")
    else:
        for section in SECTIONS:
